...
#ifdef USE_DMA										//jesli uzywa komunikacj DMA
#include <string.h>
 ...
 * @return none
 */
static void ST7789_WriteCommand(uint8_t cmd)						//Funkcja odpowiedzialna za wysłanie komendy
{
	ST7789_Select();
	ST7789_DC_Clr();
 ...
 * @return none
 */
static void ST7789_WriteData(uint8_t *buff, size_t buff_size)				//Przesłanie wiecej niz 8 bitów
{
	ST7789_Select();
	ST7789_DC_Set();
	...
	while (buff_size > 0) {
		uint16_t chunk_size = buff_size > 65535 ? 65535 : buff_size;		//Jesli buff_size wiekszy od 16bit chunk_size ma 16 bit jak nie to rozmiar buforu
		#ifdef USE_DMA
		...
		#else
			HAL_SPI_Transmit(&ST7789_SPI_PORT, buff, chunk_size, HAL_MAX_DELAY);	//wysłanie pierwszej czesci bufforu
		#endif
		buff += chunk_size;								//przesuniecie bufforu
		buff_size -= chunk_size;							//mniejszenie wielkosci bufforu
	}											//Funckja bedzie sie wykonywała do pukio nie zopstanie przesłany cały bufor w paczkach 16 bitowych
 ...
 * @return none
 */
static void ST7789_WriteSmallData(uint8_t data)							//wysłąnie danych w paczce 8bit
{
	ST7789_Select();
	ST7789_DC_Set();
 ...
 * @return none
 */
void ST7789_SetRotation(uint8_t m)								//Ustawienie rotacji ekarnu
{
 ...
 * @return none
 */
static void ST7789_SetAddressWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)		//ustawienie pola do np wypełnienia(pola na kt wykonane zostaną nastepne operacje
{
	ST7789_Select();
	...
	/* Write to RAM */
	ST7789_WriteCommand(ST7789_RAMWR);							//zapis do RAMu wyswietlacza
	ST7789_UnSelect();
}


/****USTAWIENIE POCZĄTKOWE EKRANU ZAWSZE UZYC NA POCZATKU PROGRAMU****/
...
//wypełnienie CAŁEGO ekranu jakims kolorem
void ST7789_Fill_Color(uint16_t color)
{
	uint16_t i;
	ST7789_SetAddressWindow(0, 0, ST7789_WIDTH - 1, ST7789_HEIGHT - 1);			//ustawienie pola do wypełnienia jako cały ekran
	ST7789_Select();
